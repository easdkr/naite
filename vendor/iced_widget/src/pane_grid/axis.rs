...
impl Axis {
    ...
    pub fn split(&self, rectangle: &Rectangle, ratio: f32, spacing: f32) -> (Rectangle, Rectangle) {
        match self {
            Axis::Horizontal => {
                let height_top = (rectangle.height * ratio - spacing / 2.0).round();
            ...
            Axis::Vertical => {
                let width_left = (rectangle.width * ratio - spacing / 2.0).round();
