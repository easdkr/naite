...
use crate::core::{
    self, Clipboard, Element, Length, Point, Rectangle, Shell, Size, Vector, Widget,
};
...
pub trait Component<Message, Theme = crate::Theme, Renderer = crate::Renderer> {
    ...
    fn operate(&self, _state: &mut Self::State, _operation: &mut dyn widget::Operation) {}
...
impl<'a, Message, Theme, Renderer, Event, S> Instance<'a, Message, Theme, Renderer, Event, S>
where
    S: Default + 'static,
    Renderer: renderer::Renderer,
{
    ...
    fn rebuild_element_with_operation(&self, operation: &mut dyn widget::Operation) {
...
impl<'a, 'b, Message, Theme, Renderer, Event, S> overlay::Overlay<Message, Theme, Renderer>
    ...
    S: 'static + Default,
{
    ...
    fn on_event(
        ...
        let event_status = self
            .with_overlay_mut_maybe(|overlay| {
                overlay.on_event(event, layout, cursor, renderer, clipboard, &mut local_shell)
            })
    ...
    fn is_over(&self, layout: Layout<'_>, renderer: &Renderer, cursor_position: Point) -> bool {
        self.with_overlay_maybe(|overlay| overlay.is_over(layout, renderer, cursor_position))
            .unwrap_or_default()
    }
}
