                ...
                match event {
                    ...
                    event::Event::WindowEvent {
                        ...
                        if new_mouse_interaction != window.mouse_interaction {
                            window
                                .raw
                                .set_cursor(conversion::mouse_interaction(new_mouse_interaction));
                        ...
                        let _ = control_sender.start_send(Control::ChangeFlow(match ui_state {
                    ...
                    event::Event::AboutToWait => {
                        ...
                        if !messages.is_empty() || uis_stale {
                            let cached_interfaces: FxHashMap<window::Id, user_interface::Cache> =
                                ManuallyDrop::into_inner(user_interfaces)
                                    .drain()
                                    .map(|(id, ui)| (id, ui.into_cache()))
                                    .collect();

                            update(&mut program, &mut runtime, &mut debug, &mut messages);
...
fn run_action<P, C>(
    ...
    debug: &mut Debug,
    interfaces: &mut FxHashMap<window::Id, UserInterface<'_, P::Message, P::Theme, P::Renderer>>,
    ...
    match action {
        ...
        Action::Window(action) => match action {
            ...
            window::Action::ShowSystemMenu(id) => {
                if let Some(window) = window_manager.get_mut(id) {
                    if let mouse::Cursor::Available(point) = window.state.cursor() {
