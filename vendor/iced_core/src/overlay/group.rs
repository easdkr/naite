    ...
    Renderer: 'a + crate::Renderer,
{
    ...
    pub fn with_children(children: Vec<overlay::Element<'a, Message, Theme, Renderer>>) -> Self {
        Group { children }
    }
    ...
    Renderer: crate::Renderer,
{
    ...
    fn on_event(
        ...
        self.children
            ...
            .zip(layout.children())
            .map(|(child, layout)| {
                child.on_event(event.clone(), layout, cursor, renderer, clipboard, shell)
            })
    ...
    fn mouse_interaction(
        ...
        self.children
            .iter()
            .zip(layout.children())
            .map(|(child, layout)| child.mouse_interaction(layout, cursor, viewport, renderer))
            .max()
            .unwrap_or_default()
    }
    ...
    fn is_over(&self, layout: Layout<'_>, renderer: &Renderer, cursor_position: Point) -> bool {
        self.children
            .iter()
            .zip(layout.children())
            .any(|(child, layout)| child.is_over(layout, renderer, cursor_position))
    }
