...
macro_rules! color {
    ...
    ($r:expr, $g:expr, $b:expr, $a:expr) => {{
        ...
        {
            debug_assert!(r >= 0.0 && r <= 1.0, "R channel must be in [0, 255] range.");
            debug_assert!(g >= 0.0 && g <= 1.0, "G channel must be in [0, 255] range.");
            debug_assert!(b >= 0.0 && b <= 1.0, "B channel must be in [0, 255] range.");
        }
